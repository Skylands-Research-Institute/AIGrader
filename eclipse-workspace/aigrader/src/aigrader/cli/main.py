...
if __name__ == "__main__" and __package__ is None:
    ...
    from aigrader.batch import AssignmentSpec, BatchGrader, load_assignment_file
    ...
    from aigrader.idempotency import compute_submission_fingerprint, already_assessed, get_fingerprint_marker
    ...
    from ..batch import AssignmentSpec, BatchGrader, load_assignment_file
    ...
    from ..idempotency import compute_submission_fingerprint, already_assessed, get_fingerprint_marker
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description="AIGrader - AI-assisted grading for Canvas LMS"
    )
...
def grade_one_assignment(
    ...
    print(f"\nAssignment: {run.preflight.assignment_name}")
    ...
    if already and not args.force and not args.print_prompts:
        print("\nSKIP: Submission already assessed (no changes detected)")
    ...
    # -----------------------------
    assignment_description = client.get_assignment_description(
        course_id=course_id,
        assignment_id=assignment_id
    )
    ...
    if already and not args.force:
        print("\nNOTE: Already assessed; printing prompts only")
        print(f"Fingerprint: {fp}")
        return 0
    ...
    if args.use_llm:
        ...
        if args.save_raw:
            ...
            if "{" in save_path:
                ...
                if ext:
                    ...
                    save_path = f"{save_path}_{course_id}_{assignment_id}.txt"
            
            with open(save_path, "w", encoding="utf-8") as f:
                f.write(raw_text)
            print(f"Saved raw output: {save_path}")
        ...
        for c in run.rubric.criteria:
            criteria_obj[c.id] = {
                ...
                "comment": f"Strong work on {c.description.lower()}."
            }
        ...
        raw_text = json.dumps(
            {
                "overall_score": total,
                "overall_comment": "Mock assessment - perfect scores.",
...
def main() -> int:
    ...
    if args.course_id is None or args.assignment_id is None:
        raise RuntimeError(
            "Missing --course-id/--assignment-id (or use --assignment-file for batch mode)"
        )
